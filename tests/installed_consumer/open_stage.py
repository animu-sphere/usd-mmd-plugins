...
SkelRoot, the mesh bound to the skeleton, a material with the installed
MmdMaterialAPI applied, and its texture resolving beside the file
...
assert asset.GetCustomDataByKey("mmd:stageContractVersion") == 2
...
assert hair.HasAPI("MmdMaterialAPI"), f"MmdMaterialAPI is not applied: {hair.GetAppliedSchemas()}"
texture = hair.GetAttribute("inputs:mmd:material:texture").Get()
